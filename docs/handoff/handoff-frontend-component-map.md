...
| CreateLearningTaskForm | `components/teacher/CreateLearningTaskForm.tsx` | `POST learning-tasks/tasks` | 只负责模板创建（核心字段 + 可选 `courseLabel` + 基础 rubric 配置，四维中文展示口径复用 `lib/ui/rubric.ts`），不负责班级实例发布 |
| EditLearningTaskForm | `components/teacher/EditLearningTaskForm.tsx` | `PATCH learning-tasks/tasks/:id` | 只负责模板编辑（核心字段 + 可选 `courseLabel` + status + rubric，四维中文展示口径复用 `lib/ui/rubric.ts`），不负责班级实例发布 |
| LearningTaskFilters | `components/teacher/LearningTaskFilters.tsx` | `GET learning-tasks/tasks?courseLabel=...`（通过 URL query 触发服务端列表过滤） | 负责模板层筛选与列表呈现（`status/knowledgeModule/stage` 本地筛选 + `courseLabel` URL 筛选），不发起模板发布请求 |
