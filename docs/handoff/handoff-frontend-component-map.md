...
| LearningTaskFilters | `components/teacher/LearningTaskFilters.tsx` | `GET learning-tasks/tasks?scope=...&courseLabel=...`（通过 URL query 触发服务端列表过滤） | 负责模板层视图与筛选（`scope` 视图切换 + `status/knowledgeModule/stage` 本地筛选 + `courseLabel` URL 筛选）与列表呈现（含 `visibility` 轻量标签、非作者模板操作边界）；编辑/查看入口会附带 `returnTo`（当前列表完整 URL）以保留回跳上下文；默认排序在“筛选后、渲染前”按 scope 策略执行；不发起模板发布请求 |
