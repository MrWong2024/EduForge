...
| AiLearningAnalyticsSummary / AiLearningAnalyticsTeachingAttention | `components/teacher/AiLearningAnalyticsSummary.tsx` | - | 纯 Server 展示组件；组合确定性可读摘要与“覆盖 / 响应 / 结果”三段式摘要，并仅按已映射 `taskTrends` 选择最低反馈后重提率、最多恶化样本、最高改善率任务；严格保留后端分母，不重算业务聚合，不生成评分或风险等级。 |
| AiLearningAnalyticsTaskTable / AiLearningAnalyticsStudentsTable | `components/teacher/AiLearningAnalyticsTables.tsx` | - | 压缩课堂任务表与学生分析表，合并重复列并 sticky 固定首列；保留任务原顺序、学生详情 query 和零样本表达，不提供排序、搜索、筛选或排名。 |
| AiLearningAnalyticsCharts | `components/teacher/AiLearningAnalyticsCharts.tsx` | - | 纯展示原生 SVG；班级与个人复用逐任务横向 before/after 问题负荷坐标逻辑，班级另展示以任务质量可比样本为分母的改善/持平/恶化堆叠分布；不连接不同任务，不把 null 绘制为 0，长任务名完整换行，并提供形状/纹理、SVG title 与完整可访问文本；无 API、无后端统计算法或动画依赖。 |
| AiLearningAnalyticsMethodologyPanel / AiLearningAnalyticsMetricGuide | `components/teacher/AiLearningAnalyticsMethodology.tsx` | - | 常显方法学面板继续展示分析范围、样本单位、质量代理、后端 disclaimer、任务难度未校正及非成绩/非因果边界；共享指标说明明确“总体变化”不是时间序列，只反映当前范围内可比任务平均问题负荷差值方向，并说明单个可比任务不代表成长趋势及 V1 STABLE 尚未拆分前后均为 0 与同负荷非 0 情形；不承担业务计算。 |
