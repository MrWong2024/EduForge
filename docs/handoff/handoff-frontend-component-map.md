...
| AiLearningAnalyticsStudentFilters | `components/teacher/AiLearningAnalyticsStudentFilters.tsx` | 通过页面 URL 触发 students GET | 纯 Server Component 原生 GET form；承载姓名/学号搜索、总体结果与反馈参与阶段单选筛选，hidden 保留 `window/excludedTaskIds`，应用和清空均回到第 1 页；无浏览器状态、无当前页过滤。 |
