...
| SubmissionAutoRefresh | `components/student/SubmissionAutoRefresh.tsx` | `router.refresh()`（复用 submission detail 页面现有读源链路） | 仅用于学生提交详情页状态驱动自动刷新；`PENDING/RUNNING` 快速、`FAILED` 慢速；页面失焦/不可见暂停；同页实例内防重叠 |
