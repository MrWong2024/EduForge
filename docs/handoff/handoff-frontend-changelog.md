...
- 【新增事实 / 已收口口径】空系统起步引导已补齐；核心页 raw JSON 采用默认折叠。

## Step 12A

- 【本步解决】课程创建表单学期 placeholder 长期固定示例与当前教学季不贴合的问题。
- 【新增事实 / 已收口口径】`CreateCourseForm` 的学期输入框 placeholder 已改为前端按当前月份动态生成：1 月显示上一年秋季，2-7 月显示当年春季，8-12 月显示当年秋季。
- 【边界保持】该变化仅影响灰色示例提示，不自动填充 term，不修改课程创建 payload、校验或后端契约。

## Step 12B

- 【本步解决】班级创建表单班级名称 placeholder 长期固定示例与当前教学季不贴合的问题。
- 【新增事实 / 已收口口径】`CreateClassroomForm` 的班级名称输入框 placeholder 已改为前端按当前月份动态生成：1 月显示上一年秋季班级示例，2-7 月显示当年春季班级示例，8-12 月显示当年秋季班级示例。
- 【边界保持】该变化仅影响灰色示例提示，不自动填充班级名称，不修改班级创建 payload、校验或后端契约。
- 【不要回退】不要恢复“主视图缺信息、必须先看 raw JSON 才能操作”的旧体验。
- 【对新会话的意义】验收与续接可直接基于 snapshot / route-map / component-map，不需回忆会话过程。
...
- 【本步解决】学生提交表单缺少多文件 `codeText` 粘贴格式提示，学生不清楚何时需要 `FILE` 文本分隔的问题。
- 【新增事实 / 已收口口径】`SubmissionForm` 的 `codeText` 输入区已新增轻量提示：普通单文件代码可直接粘贴提交，无需添加 `FILE` 标记；当代码分散在多个文件中，可按推荐标准格式 `===== FILE: relative/path/FileName.ext =====` 粘贴，并用语言无关的双文件示例说明，文件名可替换为自己的文件名或相对路径。
- 【不要回退】不要把该提示改成上传文件、多文件编辑器、结构化文件提交或前端自动解析能力说明；不要在主示例中使用具体语言完整代码。
- 【对新会话的意义】学生端已具备最小必要提示，可在保持现有 `content.codeText` 提交流程不变的前提下，降低多文件粘贴误用成本。
...
- 【本步解决】教师班级任务列表的 `ACTIVE` 长句说明过重，且缺少教师扫读所需的提交窗口提示。
- 【新增事实 / 已收口口径】`/teacher/classrooms/[classroomId]/tasks` 保持生命周期状态“开放中/已关闭/已撤回”，并在任务状态列内新增提交窗口辅助标签：可提交、允许迟交、不可提交、状态未知。
- 【边界保持】提交窗口标签仅按 `classroomTask.status + dueAt + allowLate` 做前端列表提示，不替代后端提交权限，不改变截止时间状态、迟交规则、关闭/撤回/恢复操作。不改 backend，不新增依赖。

## UAT-FE-79

- 【本步解决】教师班级任务列表的课堂任务级“AI 状态”列长期显示低价值占位，容易造成表格噪声。
- 【新增事实 / 已收口口径】`/teacher/classrooms/[classroomId]/tasks` 已移除“AI 状态”表头与行单元格；classroomTask 不再展示单一 AI 状态。
- 【边界保持】三件套入口中的“AI 指标”仍保留，教师查看 AI 情况继续下钻到 ai-metrics；不新增 AI 聚合统计，不改 backend，不新增依赖。

## UAT-FE-80

- 【本步解决】课堂任务工作区子页面顶部缺少返回班级级中枢的统一入口，教师从提交记录页或任务分析三件套页回班级看板路径不直观的问题。
- 【新增事实 / 已收口口径】`/teacher/classrooms/[classroomId]/tasks/[classroomTaskId]/submissions`、`/learning-trajectory`、`/review-pack`、`/ai-metrics` 顶部导航均已补齐“班级看板”入口，统一跳转 `paths.teacher.classroomDashboard(classroomId)`。
- 【导航顺序】提交记录页顶部顺序收口为“班级看板 -> 返回任务列表 -> 返回任务详情 -> 学习轨迹 -> 课堂复盘 -> AI 指标”；三件套页顶部顺序统一为“班级看板 -> 返回任务列表 -> 提交管理”。
- 【不要回退】不要删除提交记录页现有三件套入口，不要删除三件套页现有“返回任务列表”“提交管理”，不要把本次补强扩展成班级看板或任务列表入口改造。
- 【边界保持】仅前端导航补强，不新增接口、不改数据加载/筛选/图表/表格/空态/错误态/加载态逻辑，不改 backend，不新增依赖。

## UAT-FE-81

- 【本步解决】教师班级成员页此前只依赖后端默认分页大小，成员较多时无法稳定查看后续成员的问题。
- 【新增事实 / 已收口口径】`/teacher/classrooms/[classroomId]/members` 现已显式请求 `page` 与固定 `limit=100`，并展示数量摘要“共 X 名成员，当前显示 Y 名”。
- 【分页收口】当 `total <= 100` 时不显示分页按钮；当 `total > 100` 时显示轻量分页“第 N / M 页 / 上一页 / 下一页”；`includeRemoved` 切换时回到 `page=1`。
- 【不要回退】不要恢复仅依赖后端默认分页大小的请求方式，不要在本页扩展为搜索、每页数量切换或展开全部。
- 【边界保持】仅前端成员页轻量分页接入，不改 backend，不新增接口，只在原成员接口请求中补充 `page/limit` 参数，不改变成员移除/恢复语义。

## UAT-FE-82

- 【本步解决】教师提交详情页顶部“返回班级列表”层级过高，不贴合“班级 -> 课堂任务 -> 提交 -> 批阅”的真实工作流问题。
- 【新增事实 / 已收口口径】`/teacher/submissions/[submissionId]` 顶部导航已调整为：`班级看板 -> 返回任务提交列表 -> 返回任务详情`；其中“班级看板”仅在 query 存在 `classroomId` 时显示，统一跳转 `paths.teacher.classroomDashboard(classroomId)`。
- 【替换口径】原 `返回班级列表` 已移除；缺少 `classroomId` 时不显示 `班级看板`，且不 fallback 到班级列表。
- 【不要回退】不要新增 `返回课堂复盘`、`返回 AI 指标`、`返回学习轨迹`，也不要为 review-pack 增加来源追踪参数。
- 【边界保持】仅前端导航补强，不改 backend，不新增接口，不改变提交详情/教师批阅的数据加载、反馈提交、表单、错误态或加载态逻辑。

## UAT-FE-83

- 【本步解决】教师学习轨迹页此前默认 `limit=20`、URL `limit` 最大仅 `50`，且总数较少时仍显示基础分页入口，分页体验不够轻量的问题。
- 【新增事实 / 已收口口径】`/teacher/classrooms/[classroomId]/tasks/[classroomTaskId]/learning-trajectory` 现已默认请求 `limit=100`，URL `limit` 最大允许 `100`，并在学生列表区域展示“共 X 名学生，当前显示 Y 名”。
- 【分页收口】当 `total <= limit` 时不显示分页按钮与 `第 1 / 1 页`；当 `total > limit` 时显示轻量分页“第 N / M 页 / 上一页 / 下一页”，翻页保留 `window/sort/order/includeAttempts/includeTagDetails/limit` 等当前 query 状态。
- 【不要回退】不要把本页扩展成搜索、每页数量切换、展开全部/收起，也不要改动课堂复盘、AI 指标、提交记录或班级成员页的分页口径。
- 【边界保持】仅前端学习轨迹页轻量分页体验优化，不改 backend，不新增接口，只调整原学习轨迹接口请求的 `limit` 与分页展示，不改变图表、表格、空态、错误态或加载态主逻辑。

## UAT-FE-84

- 【本步解决】教师任务提交管理页此前固定请求 `page=1&limit=50`，不读取 URL `page`，且缺少总数摘要与分页入口，导致超过 50 条提交后无法继续查看的问题。
- 【新增事实 / 已收口口径】`/teacher/classrooms/[classroomId]/tasks/[classroomTaskId]/submissions` 现已显式读取 URL `page`，并固定请求 `page={当前页}&limit=100`；表格区域展示“共 X 条提交，当前显示 Y 条”。
- 【分页收口】当 `total <= 100` 时不显示分页按钮与 `第 1 / 1 页`；当 `total > 100` 时显示轻量分页“第 N / M 页 / 上一页 / 下一页”。
- 【不要回退】不要把本页扩展成搜索、筛选、每页数量切换、展开全部/收起，也不要改动顶部导航、表格列或“查看详情 / 批阅”入口。
- 【边界保持】仅前端提交管理页轻量分页体验优化，不改 backend，不新增接口，只调整原提交列表接口请求的 `page/limit` 与分页展示，不改变排序、空态、错误态或加载态逻辑。

## UAT-FE-85

- 【本步解决】教师编辑班级页仍保留过期的“当前阶段不包含删除与归档动作”提示，且表单区内存在“危险操作 / 归档班级”区块，容易让用户误以为编辑页承担低频风险操作。
- 【新增事实 / 已收口口径】`/teacher/classrooms/[classroomId]/edit` 现仅保留中性说明：第一行“此页用于维护班级基础信息。”；第二行明确“修改班级信息不会影响班级成员、课堂任务和历史提交；归档、删除等低频操作请在班级列表的‘更多’菜单中处理。”。
- 【界面收口】编辑页不再显示“危险操作”标题、红色危险区块或“归档班级”按钮；顶部导航、编辑表单字段与保存逻辑保持不变。
- 【不要回退】不要在编辑页重新加入归档/删除入口，不要恢复“当前阶段仅接入后端已开放的更新能力，不包含班级删除与归档动作。”旧文案。
- 【边界保持】仅前端文案与危险区块清理，不改 backend，不改班级列表“更多”菜单，不改编辑表单提交接口与错误/加载逻辑。

## UAT-FE-86

- 【本步解决】教师过程性评价页此前未显式传 `page/limit`，默认只使用后端默认分页大小，且明细表缺少轻量分页入口，超过 50 条学生明细后无法继续查看的问题。
- 【新增事实 / 已收口口径】`/teacher/classrooms/[classroomId]/process-assessment` 现已显式读取 URL `page`，并固定请求 `page={当前页}&limit=100`；切换 `window` 时回到 `page=1`；明细区展示“共 X 名学生，当前显示 Y 名”。
- 【分页收口】当 `total <= 100` 时不显示分页按钮与 `第 1 / 1 页`；当 `total > 100` 时显示轻量分页“第 N / M 页 / 上一页 / 下一页”；明细说明更新为“超过 100 人时可翻页查看，完整结果可通过 CSV 导出。”。
- 【不要回退】不要把本页扩展成搜索、每页数量切换、展开全部/收起，也不要改变 CSV 导出语义或把摘要改成全班全量统计。
- 【边界保持】仅前端过程性评价页轻量分页体验优化，不改 backend，不新增接口，只调整原过程性评价接口请求的 `page/limit` 与明细分页展示；摘要聚合口径仍基于当前窗口与当前返回明细。

## UAT-FE-87

- 【本步解决】教师课程列表与班级列表此前默认每页仅 20 条，且即使无实际分页也保留灰色“上一页/下一页”文字，导致大多数列表页体验不够接近“一页看完”。
- 【新增事实 / 已收口口径】`/teacher/courses` 与 `/teacher/classrooms` 主列表现已默认请求 `page=1&limit=100`；课程列表显示“共 X 门课程，当前显示 Y 门”，班级列表显示“共 X 个班级，当前显示 Y 个”。
- 【分页收口】当 `total <= 100` 时不显示分页按钮与 `第 1 / 1 页`；当 `total > 100` 时显示轻量分页“第 N / M 页 / 上一页 / 下一页”。
- 【筛选联动】课程状态筛选 `进行中/已归档/全部` 切换时回到 `page=1`；班级状态筛选与 `courseId` 筛选切换时也回到 `page=1`，同时保留当前另一个筛选项。
- 【边界保持】仅前端课程/班级列表分页体验优化，不改 backend，不新增接口，不做无限制全量查询；班级列表用于课程筛选/名称映射的辅助请求 `courses?page=1&limit=100` 保持原样，不新增搜索/每页数量切换/展开全部。

## UAT-FE-88

- 【本步解决】课程总览页班级明细此前默认每页 20 条、URL `limit` 最大 50，且无实际分页时仍展示灰色分页文字，和近期列表页轻量分页口径不一致。
- 【新增事实 / 已收口口径】`/teacher/courses/[courseId]/overview` 班级明细现已默认请求 `page=1&limit=100`，URL `limit` 最大允许 `100`；明细区展示“共 X 个班级，当前显示 Y 个”。
- 【分页收口】当 `total <= limit` 时不显示分页按钮与 `第 1 / 1 页`；当 `total > limit` 时显示轻量分页“第 N / M 页 / 上一页 / 下一页”，翻页保留 `window/sort/order/limit`。
- 【筛选联动】切换统计窗口、排序字段、排序方向时回到 `page=1`，继续保留既有 `window/sort/order` 语义。
- 【边界保持】仅前端课程总览页班级明细分页体验优化，不改 backend，不新增接口，不做无限制全量查询；摘要聚合口径仍基于当前页班级明细，不新增搜索/每页数量切换/展开全部。
...
- 【新增事实 / 已收口口径】`/teacher/classrooms/[classroomId]/process-assessment` 已新增“排除任务（临时计算）”区域，由 `ExcludeTasksPanel` Client Component 使用课堂任务列表接口加载选项；应用排除通过 `router.replace` 客户端软导航将当前选中的 `excludedTaskIds` 写入 URL 后触发重新计算；页面解析兼容逗号分隔与 repeated query，向过程性评价 JSON 与 CSV 下载请求时统一透传逗号分隔值。
...
- 【本步解决】过程性评价页“排除任务 / 清空排除”旧 GET form 交互在 hydration 场景下存在不稳定风险，且后端评分已升级为任务维度指标。
