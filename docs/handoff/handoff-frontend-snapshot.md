...
3. 明细表已显式增加“得分”列（展示 `score`）；并继续使用 view-model 友好化渲染：统一学生显示名、进度百分比、风险中文标签（高/中/低风险）与备注/问题摘要，不再在 JSX 中堆叠多层 `safeGet(...)` 兜底表达。
4. `topTags` 问题摘要已统一使用英文原标签（不再做不完整中文映射）；备注列优先展示真实备注，无备注时回退“主要问题：tag (count)”并默认最多展示前 3 个标签方向，避免中英混用。
