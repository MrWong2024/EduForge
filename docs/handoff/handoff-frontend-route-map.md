...
| `/teacher/submissions/[submissionId]` | 教师提交详情/批阅 | `GET learning-tasks/submissions/:id`、`GET learning-tasks/submissions/:id/feedback`、`POST learning-tasks/submissions/:id/feedback`、`PATCH learning-tasks/submissions/:submissionId/feedback/:feedbackId` | 查看代码与反馈、新增教师反馈；反馈历史中仅 `source=TEACHER` 且有 `id` 的条目显示“修改”，支持原地编辑并保存后 `router.refresh()`；AI/SYSTEM 反馈只读 | Done | 稳定读源 + 真接口 |
