...
| `/teacher/tasks/[taskId]/edit` | 任务模板编辑/查看页 | `GET learning-tasks/tasks/:id`、`PATCH learning-tasks/tasks/:id`、`POST learning-tasks/tasks/:id/restore` | 作者可编辑 DRAFT/PUBLISHED 模板核心字段（含可选 `courseLabel`、`visibility`）、维护 rubric 基础配置；作者打开 `ARCHIVED` 模板时页面只读，提示先恢复为草稿，恢复成功后刷新进入 DRAFT 可编辑态；非作者共享模板进入只读查看模式且不显示恢复按钮，并基于 `publisher` 显示模板发布者；保存失败时，若后端返回“已被课堂任务引用的 `PUBLISHED` 模板不能改回 `DRAFT`”错误，前端显示中文友好提示而不是裸英文 detail；支持 `returnTo` 回跳上下文（优先返回原模板列表 URL，缺失/非法时回退 `/teacher/tasks`） | Done | 真接口 |
