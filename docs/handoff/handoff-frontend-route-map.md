...
| `/teacher/classrooms/[classroomId]/tasks` | 班级任务列表/发布页（班级实例层） | `GET classrooms/:id`、`GET classrooms/:id/tasks`、`GET learning-tasks/tasks?status=PUBLISHED...`、`POST classrooms/:id/tasks` | 候选模板本地筛选（module/stage）、选择已发布模板、配置 `dueAt/allowLate/maxAttempts` 并发布实例、进入详情/提交/三件套、跳模板页 | Done | 真接口 |
...
| `/teacher/classrooms/[classroomId]/tasks/[classroomTaskId]/learning-trajectory` | 学习轨迹 | `GET .../learning-trajectory` | window/sort/order/include* 切换 | Done | 真接口 |
...
| `/student/classrooms/[classroomId]/tasks/[classroomTaskId]` | 学生任务详情 | `GET .../my-task-detail`、`POST .../submissions` | 基于 `my-task-detail` 聚合结果展示我的任务详情/历史提交入口，并执行提交后跳 submission detail | Done | 真接口 |
