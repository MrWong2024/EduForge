...
| `/teacher/classrooms/[classroomId]/ai-learning-analytics` | AI 反馈介入成效分析 | `GET classrooms/:classroomId/ai-learning-analytics`、`GET classrooms/:classroomId/ai-learning-analytics/students`、`GET classrooms/:classroomId/tasks`（排除选项） | Server Component；页面顺序为 PageHeader -> 常显方法学/免责声明 -> 统计窗口 -> 临时排除任务 -> 后端 summary 摘要 -> 班级反馈介入变化趋势 -> 全任务数值表 -> ACTIVE 学生分页列表 -> 默认收起 raw JSON。主窗口 `7d/30d/all`（默认 `all`）；`excludedTaskIds` 兼容逗号/repeated 输入、请求时归一为逗号，窗口或排除变化回到 `page=1`；学生固定 `limit=20`，翻页保留窗口与排除。任务选项失败只警告，学生列表失败只影响局部，总览失败为页面错误；零任务/学生/提交/请求/交付/可比样本均有独立空态。班级看板 PageHeader 已提供“AI 成效分析”入口。页面只表达反馈介入变化，不作成绩、能力增长或因果结论。 | Done | 真接口 |
| `/teacher/classrooms/[classroomId]/ai-learning-analytics/students/[studentId]` | 单学生 AI 反馈介入分析 | `GET classrooms/:classroomId/ai-learning-analytics/students/:studentId` | Server Component；结构为 PageHeader -> 常显方法学/免责声明 -> 学生摘要 -> 个人反馈介入变化轨迹 -> 全任务明细 -> 默认收起 raw JSON。详情沿用 `window/excludedTaskIds`，URL `page` 仅用于“返回班级分析”恢复分页，不传后端；返回链接完整保留三项状态。404 使用“学生不存在或无权访问”中性表达；无可比任务时保留摘要和全任务表，仅曲线显示专用空态。 | Done | 真接口 |
