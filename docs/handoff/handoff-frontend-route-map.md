...
| `/teacher/classrooms/[classroomId]/ai-learning-analytics` | AI 反馈介入成效分析 | `GET classrooms/:classroomId/ai-learning-analytics`、`GET classrooms/:classroomId/ai-learning-analytics/students`、`GET classrooms/:classroomId/tasks`（排除选项） | Server Component；页面在方法学、指标说明、统计窗口和临时任务排除之后，展示确定性班级摘要与“覆盖 / 响应 / 结果”三段式摘要、最多三条仅按 `taskTrends` 极值选择的教学关注提示、课堂任务对比分析、压缩后的任务明细与 ACTIVE 学生分页列表。班级图表为“各任务 AI 反馈前后问题对比”和“各任务可比样本结果分布”，不再用折线连接不同任务或混合不同分母比例；不可比较任务不绘制为 0。任务表与学生表合并重复列并 sticky 固定第一列，学生字段使用“总体变化”且显示可比任务数，该结论不是严格时间趋势。主窗口、`excludedTaskIds`、`page` 与固定 `limit=100` 行为保持不变；raw JSON 仅在非生产环境默认收起显示。任务选项失败只警告，学生列表失败只影响局部，总览失败为页面错误；零任务/学生/提交/请求/交付/可比样本继续使用对应空态。 | Done | 真接口 |
| `/teacher/classrooms/[classroomId]/ai-learning-analytics/students/[studentId]` | 单学生 AI 反馈介入分析 | `GET classrooms/:classroomId/ai-learning-analytics/students/:studentId` | Server Component；学生摘要使用“总体变化”并显示结论基于的质量可比任务数；个人图表改为“个人各任务 AI 反馈前后问题对比”，逐任务展示 before/after、差值与 outcome，不连接不同课堂任务，不可比较任务不绘制为 0，无可比任务时保留原 EmptyState。详情继续沿用 `window/excludedTaskIds`，URL `page` 仅用于返回班级分析恢复分页且不传详情接口；raw JSON 仅在非生产环境默认收起显示。 | Done | 真接口 |
