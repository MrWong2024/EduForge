...
| `/teacher/classrooms/[classroomId]/ai-learning-analytics` | AI 反馈介入成效分析 | `GET classrooms/:classroomId/ai-learning-analytics`、`GET classrooms/:classroomId/ai-learning-analytics/students`、`GET classrooms/:classroomId/tasks`（排除选项） | Server Component；页面顺序为 PageHeader -> 常显方法学/免责声明 -> 默认收起“指标与图表说明” -> 统计窗口 -> 临时排除任务 -> 后端 summary 摘要 -> 班级图表 -> 全任务数值表 -> ACTIVE 学生分页列表 -> 默认收起 raw JSON。主窗口 `7d/30d/all`（默认 `all`），`7d/30d` 展示文案明确按课堂任务发布时间筛选，任务纳入后完整提交链参与分析；`excludedTaskIds` 兼容逗号/repeated 输入、请求时归一为逗号，窗口或排除变化回到 `page=1`；学生固定 `limit=100`，翻页保留窗口与排除。班级图表标题为“AI 反馈前后问题变化”“反馈后的提交与改善情况”；共享规则说明覆盖问题负荷、质量可比样本、比例分母、缺口非真实 0 及非成绩/非能力/非因果边界。任务选项失败只警告，学生列表失败只影响局部，总览失败为页面错误；零任务/学生/提交/请求/交付/可比样本均有独立空态。班级看板 PageHeader 已提供“AI 成效分析”入口。 | Done | 真接口 |
| `/teacher/classrooms/[classroomId]/ai-learning-analytics/students/[studentId]` | 单学生 AI 反馈介入分析 | `GET classrooms/:classroomId/ai-learning-analytics/students/:studentId` | Server Component；结构为 PageHeader -> 常显方法学/免责声明 -> 默认收起“指标与图表说明” -> 学生摘要 -> “个人 AI 反馈前后问题变化”图 -> 全任务明细 -> 默认收起 raw JSON。共享规则说明位于图表前，解释问题负荷、质量可比样本、比例分母、窗口边界与介入变化趋势的非时间回归/非能力结论。详情沿用 `window/excludedTaskIds`，窗口标签同样按课堂任务发布时间表达；URL `page` 仅用于“返回班级分析”恢复分页，不传后端，返回链接完整保留三项状态。404 使用“学生不存在或无权访问”中性表达；无可比任务时保留摘要和全任务表，仅曲线显示专用空态。 | Done | 真接口 |
