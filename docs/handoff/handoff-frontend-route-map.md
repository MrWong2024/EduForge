...
| `/teacher/classrooms` | 班级列表 + 创建班级 | `GET classrooms`、`GET courses`、`POST classrooms`、`PATCH classrooms/:id`、`DELETE classrooms/:id` | 默认按 `进行中/已归档/全部` 视图区分展示（`statusView` query 驱动；列表请求透传 `status`）；支持创建班级；行内支持归档（`status=ARCHIVED`）/恢复（`status=ACTIVE`）；删除作为次级操作，命中 `409 + CLASSROOM_NOT_EMPTY` 时给出明确提示“该班级已有成员或任务记录，不能删除，只能归档”；操作后通过 `router.refresh()` 刷新并保持当前视图 query | Done | 真接口 |
