...
| `/teacher/courses/[courseId]/overview` | 课程总览 | `GET courses/:courseId/overview` | 页面结构收口为“筛选区 -> 课程摘要 -> 班级明细 -> 分页”；去除 `window/sort/order/page` 技术态裸文本回显；窗口与排序改为中文标签按钮组；新增基于现有 overview `items/total` 的轻量摘要（班级总数、当前页学生总数、有提交班级数、AI 待处理总量、AI 失败总量）；保持原有窗口切换、排序、分页与导航能力 | Done | 真接口 |
