...
- 学生看板：`/api/classrooms/mine/dashboard` 的 `classroom.teacher` 为班级级教师摘要（`id/name/employeeNo`，无 email；教师记录缺失时保留 `teacher.id` 并把文本字段回落为 `null`）；`myLatestSubmission`、`aiFeedbackStatus` 与 `completionStatus` 只围绕最终返回的 `classroomTaskId` 计算；默认隐藏归档班级、非 ACTIVE classroomTask 与长期历史任务，`includeHistorical=true` 仅放开时间窗口，不放开班级/课堂任务状态边界。
