...
- Performance Notes: 批量查询 + `Promise.all`；状态映射批量取 jobs，避免逐提交查询；`Task` schema 已补发布候选查询复合索引：`(createdBy,status,courseLabel,knowledgeModule,stage,updatedAt,createdAt)` 覆盖 `onlyMine` 分支，`(visibility,status,courseLabel,knowledgeModule,stage,updatedAt,createdAt)` 覆盖共享可见分支。
