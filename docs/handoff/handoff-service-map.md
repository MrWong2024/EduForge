...
- Actions: `manage-task-template`, `filter-task-template`, `submit`, `read-submission-detail`, `request-ai-feedback`, `feedback`, `stats`
- I/O Shape:
...
- Consistency/Constraints: late 指标含 `lateSubmissionsCount/lateStudentsCount`；默认窗口为 `all`；后端兼容窗口 `all/7d/24h/1h`；`all` 语义为“无时间下界过滤（classroomTasks/submissions/jobs 不拼 `createdAt >= lowerBound`）”；禁止跨班 taskId 兜底聚合
